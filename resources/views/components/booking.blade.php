    ...
    <div style="overflow-x: auto;">
        <div class="p-4 bg-white rounded-lg md:p-8" id="stats" role="tabpanel" aria-labelledby="stats-tab">
            <dl class="grid max-w-screen-xl grid-cols-2 gap-8 p-4 mx-auto text-gray-900 sm:grid-cols-3 xl:grid-cols-6 sm:p-8" style="min-width: max-content;">
                @forelse ($hospital->schedules as $timetable)  {{-- Use @forelse for empty collections --}}
                    <form action="/doctors/{{ $doctor->id }}/bookings/{{ $timetable->id }}" method="POST">
                        @csrf
                        <button type="submit" class="flex border border-cyan-500 px-5 rounded-lg py-2 flex-col items-center justify-center">
                            <dt class="mb-2 text-3xl font-extrabold">{{ $timetable->day }}</dt>
                            <dd class="text-gray-700 text-xl font-bold">{{ $timetable->date }}</dd>
                            <dd class="text-gray-700 text-xl font-bold">{{ $timetable->time }} : {{ $timetable->end_time }}</dd>
                        </button>
                    </form>
                @empty  {{-- Handle the case where there are no schedules --}}
